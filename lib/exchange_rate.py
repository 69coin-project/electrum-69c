...
class Bit2C(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('www.bit2c.co.il', '/Exchanges/69cNIS/Ticker.json')
...
class BitcoinAverage(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        return dict([(r.replace("69c", ""), Decimal(json[r]['last']))
    ...
    def request_history(self, ccy):
        history = self.get_csv('apiv2.bitcoinaverage.com',
                               "/indices/global/history/69c%s?period=alltime&format=csv" % ccy)
...
class BitcoinVenezuela(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        rates = [(r, json['69c'][r]) for r in json['69c']
                 if json['69c'][r] is not None]  # Giving NULL sometimes
        return dict(rates)
    ...
    def request_history(self, ccy):
        return self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=69c")[ccy +'_69c']

class Bitfinex(ExchangeBase):

    def get_rates(self, ccy):
        json = self.get_json('api.bitfinex.com', '/v1/pubticker/69cusd')
...
class Bitso(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('api.bitso.com', '/v3/ticker/?book=69c_mxn')
...
class BitStamp(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('www.bitstamp.net', '/api/v2/ticker/69cusd/')
...
class Coinbase(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('api.coinbase.com',
                             '/v2/exchange-rates?currency=69c')
...
class CoinSpot(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        return {'AUD': Decimal(json['prices']['69c']['last'])}
...
class GoCoin(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        kh69c_prices = json['prices']['69c']
        return dict([(r, Decimal(kh69c_prices[r])) for r in kh69c_prices])
...
class HitBTC(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ccys = ['EUR', 'USD']
        json = self.get_json('api.hitbtc.com', '/api/1/public/69c%s/ticker' % ccy)
...
class Kraken(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        pairs = [k for k in dicts['result'] if k.startswith('X69cZ')]
        json = self.get_json('api.kraken.com',
                             '/0/public/Ticker?pair=%s' % ','.join(pairs))
        ccys = [p[5:] for p in pairs]
        result = dict.fromkeys(ccys)
        result[ccy] = Decimal(json['result']['X69cZ'+ccy]['c'][0])
        return result
    ...
    def request_history(self, ccy):
        query = '/0/public/OHLC?pair=69c%s&interval=1440' % ccy
        json = self.get_json('api.kraken.com', query)
        history = json['result']['X69cZ'+ccy]
...
class OKCoin(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('www.okcoin.com', '/api/v1/ticker.do?symbol=69c_usd')
...
class MercadoBitcoin(ExchangeBase):
    ...
    def get_rates(self,ccy):
        json = self.get_json('www.mercadobitcoin.net', '/api/69c/ticker/')
...
class TheRockTrading(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('api.therocktrading.com', 
                             '/v1/funds/69cEUR/ticker')
        return {'EUR': Decimal(json['last'])}

class QuadrigaCX(ExchangeBase):

    def get_rates(self,ccy):
        json = self.get_json('api.quadrigacx.com', '/v2/ticker?book=69c_cad')
...
class WEX(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json_eur = self.get_json('wex.nz', '/api/3/ticker/69c_eur')
        json_rub = self.get_json('wex.nz', '/api/3/ticker/69c_rur')
        json_usd = self.get_json('wex.nz', '/api/3/ticker/69c_usd')
        return {'EUR': Decimal(json_eur['69c_eur']['last']),
                'RUB': Decimal(json_rub['69c_rur']['last']),
                'USD': Decimal(json_usd['69c_usd']['last'])}
