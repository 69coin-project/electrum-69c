...
from electrum_69c.util import format_satoshis, set_verbosity
from electrum_69c.bitcoin import is_address, COIN, TYPE_ADDRESS
...
class ElectrumGui:
    ...
    def __init__(self, config, daemon, plugins):
        ...
        if not storage.file_exists:
            print("Wallet not found. try 'electrum-69c create'")
            exit()
    ...
    def do_send(self):
        if not is_address(self.str_recipient):
            print(_('Invalid kh69coin address'))
            return
        try:
    ...
    def network_dialog(self):
        print("use 'electrum-69c setconfig server/proxy' to change your network settings")
        return True


    def settings_dialog(self):
        print("use 'electrum-69c setconfig' to change your settings")
        return True
