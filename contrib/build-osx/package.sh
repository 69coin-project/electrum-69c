...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-69c-macos
	cd /tmp/electrum-69c-macos
...
rm -rf /tmp/electrum-69c-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-69c-macos/image/
cp -r $1 /tmp/electrum-69c-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-69c_uncompressed.dmg \
    /tmp/electrum-69c-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-69c_uncompressed.dmg electrum-69c-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-69c_uncompressed.dmg

echo "Done."
md5sum electrum-69c-$VERSION.dmg
