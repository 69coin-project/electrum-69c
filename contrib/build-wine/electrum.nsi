...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-69c"
  !define PRODUCT_WEB_SITE "https://github.com/pooler/electrum-69c"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-69c-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "tmp\electrum-69c\icons\electrum.ico"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-69c-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-69c-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-69c-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-69c-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-69c-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\kh69coin" "" "URL:kh69coin Protocol"
  WriteRegStr HKCU "Software\Classes\kh69coin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\kh69coin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\kh69coin\shell\open\command" "" "$\"$INSTDIR\electrum-69c-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\kh69coin"
