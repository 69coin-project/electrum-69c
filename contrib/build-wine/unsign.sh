...
version=`python3 -c "import electrum_69c; print(electrum_69c.version.ELECTRUM_VERSION)"`
...
for mine in $(ls dist/*.exe); do
    echo "---------------"
    f=$(basename $mine)
    echo "Downloading https://electrum-69c.org/download/$f"
    wget -q https://electrum-69c.org/download/$f -O signed/$f
